// One-off: insert the curated Mediterranean (Israel's Mediterranean coast) region/sites/species from
// supabase/seed/mediterranean_seed.json into Supabase, mirroring supabase/scripts/seed_eilat.mjs
// exactly but pointed at the Mediterranean file. Idempotent (upserts by slug).
//
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node supabase/scripts/seed_mediterranean.mjs
...
const seed = JSON.parse(readFileSync(path.join(__dirname, '../seed/mediterranean_seed.json'), 'utf-8'))
...
async function main() {
  ...
  console.log(`\nSeeded ${siteIdBySlug.size} Mediterranean sites and ${speciesIdBySlug.size} Mediterranean-only species.`)
  console.log('Next: node supabase/scripts/seed.mjs (for existing-species Mediterranean site links, e.g. Common Lionfish and Common Bottlenose Dolphin), then')
