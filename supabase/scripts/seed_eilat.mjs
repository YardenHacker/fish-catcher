// One-off: insert the curated Eilat region/sites/species from
// supabase/seed/eilat_seed.json into Supabase, mirroring supabase/scripts/seed.mjs
// exactly but pointed at the Eilat file. Run after supabase/migrations/0001_init.sql
// and after the pending migration that adds sites.pro_tip (see repo notes -- until
// that migration lands, the sites upsert below will fail on the pro_tip column and
// that failure is expected). Idempotent (upserts by slug).
//
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node supabase/scripts/seed_eilat.mjs
...
const seed = JSON.parse(readFileSync(path.join(__dirname, '../seed/eilat_seed.json'), 'utf-8'))
...
async function main() {
  ...
  console.log(`\nSeeded ${siteIdBySlug.size} Eilat sites and ${speciesIdBySlug.size} Eilat-only species.`)
  console.log('Next: node supabase/scripts/seed.mjs (for existing-species Eilat site links), then')
  console.log('      node supabase/scripts/compute_rarity.mjs && node supabase/scripts/fetch_photos.mjs')
}
