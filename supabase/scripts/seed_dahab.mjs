// One-off: insert the curated Dahab region/sites/species from
// supabase/seed/dahab_seed.json into Supabase, mirroring supabase/scripts/seed_eilat.mjs
// exactly but pointed at the Dahab file. Idempotent (upserts by slug).
//
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node supabase/scripts/seed_dahab.mjs
...
const seed = JSON.parse(readFileSync(path.join(__dirname, '../seed/dahab_seed.json'), 'utf-8'))
...
async function main() {
  ...
  console.log(`\nSeeded ${siteIdBySlug.size} Dahab sites and ${speciesIdBySlug.size} Dahab-only species.`)
  console.log('Next: node supabase/scripts/seed.mjs (for existing-species Dahab site links), then')
